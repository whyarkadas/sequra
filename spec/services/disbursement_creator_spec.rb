...
RSpec.describe DisbursementCreator, type: :model do
  describe '#run' do
    ...
    it 'valid params' do
      expect {
        DisbursementCreator.new(disbursement_amount, disbursement_fee, merchant.id).run
      }.to change(Disbursement, :count).by(1)
    end

    it 'valid params, attribute check' do
      disbursement = DisbursementCreator.new(disbursement_amount, disbursement_fee, merchant.id).run
